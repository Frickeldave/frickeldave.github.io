...
# Details und Events anzeigen (zeigt Fehlerursache)
...
| ------------------ | --------------------------- | ---------------------------------------------- |
| `Pending`          | Kein Node verfügbar         | `kubectl describe pod` → Events prüfen         |
| `ImagePullBackOff` | Image nicht erreichbar      | Image-Name und Registry-Zugang prüfen          |
| `CrashLoopBackOff` | Container startet, stürzt ab | `kubectl logs <pod-name>` lesen               |
