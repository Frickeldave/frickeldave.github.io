...
| --------------------- | --------------------------------------- | ----------------------------- |
| [**App-Container**](/docs/kubernetes-basis/glossar#app-container)     | Hauptanwendung                          | Läuft dauerhaft               |
| [**Init-Container**](/docs/kubernetes-basis/glossar#init-container)    | Initialisierung vor dem App-Start       | Läuft einmalig bis Abschluss  |
| [**Sidecar-Container**](/docs/kubernetes-basis/glossar#sidecar-container) | Unterstützende Dienste (Logging, Proxy) | Läuft dauerhaft neben der App |
...
| ------------ | -------------------------------------------------------------------------------- |
| [**requests**](/docs/kubernetes-basis/glossar#resource-limits) | Mindestgarantie -- Scheduler platziert Pod nur auf Nodes mit genügend Ressourcen |
| [**limits**](/docs/kubernetes-basis/glossar#resource-limits)   | Obergrenze -- CPU wird gedrosselt, bei Speicherüberschreitung OOM-Kill           |
...
| [QoS-Klasse](/docs/kubernetes-basis/glossar#qos-klassen)     | Bedingung                               | Eviction-Priorität           |
| -------------- | --------------------------------------- | ---------------------------- |
...
Eine [LimitRange](/docs/kubernetes-basis/glossar#limitrange) setzt Standardwerte für alle Container, die keine eigenen Resources angeben.
...
| ------------------- | ------------------------------------- | ------------------------------------------ |
| [**Liveness Probe**](/docs/kubernetes-basis/glossar#liveness-probe)  | Läuft die Anwendung noch?             | Container wird neu gestartet               |
| [**Readiness Probe**](/docs/kubernetes-basis/glossar#readiness-probe) | Kann die Anwendung Traffic empfangen? | Pod wird aus Service-Endpoints entfernt    |
| [**Startup Probe**](/docs/kubernetes-basis/glossar#startup-probe)   | Ist die Anwendung fertig gestartet?   | Liveness/Readiness pausiert bis Startup OK |
...
| ----------------- | -------------------------------------------- | ----------- | ----------------------------------------------------- |
| [**RollingUpdate**](/docs/kubernetes-basis/glossar#rollingupdate) | Pods werden schrittweise ersetzt             | Keine       | Standard, Produktion                                  |
...
Eine [ConfigMap](/docs/kubernetes-basis/glossar#configmap) speichert nicht-sensible Konfigurationsdaten als Key-Value-Paare.
...
Ein [Secret](/docs/kubernetes-basis/glossar#secret) speichert sensible Daten (Passwörter, Tokens, Zertifikate). Werte werden Base64-kodiert
...
Ein [SecurityContext](/docs/kubernetes-basis/glossar#securitycontext) definiert Sicherheitseinstellungen für Pods und Container.
...
| Konzept                         | Beschreibung                                                          |
| ------------------------------- | --------------------------------------------------------------------- |
| [**PersistentVolume (PV)**](/docs/kubernetes-basis/glossar#persistentvolume)       | Ein Stück Storage im Cluster, bereitgestellt von Admin oder dynamisch |
| [**PersistentVolumeClaim (PVC)**](/docs/kubernetes-basis/glossar#persistentvolumeclaim) | Anforderung eines Pods an Storage (Größe, Zugriffsart)                |
| [**StorageClass**](/docs/kubernetes-basis/glossar#storageclass)                | Definiert, wie PVs dynamisch erstellt werden (Provisioner, Parameter) |
...
[Ingress](/docs/kubernetes-basis/glossar#ingress) stellt HTTP/HTTPS-Routing von außen in den Cluster bereit. Statt jeden Service über einen
eigenen NodePort zu exponieren, wird ein zentraler [Ingress Controller](/docs/kubernetes-basis/glossar#ingress-controller) verwendet.
