...
import knba101 from "@assets/docs/kubernetes-basis/knba-101.png";
...
- **[Self-Healing](/docs/kubernetes-basis/glossar#self-healing)** -- Automatischer Neustart ausgefallener Container
...
graph TD
    ...
    IC --> CR["Chromium<br/>Browser"]
    IC --> CS["Code-Server<br/>IDE"]
    IC --> HA["Home<br/>Assistant"]
    IC --> NC["Nextcloud"]
```
...
Kubernetes verwaltet Container genauso, wie ein Logistikzentrum Pakete verwaltet: Es entscheidet,
wo Pakete lagern, wie sie transportiert werden, und sorgt automatisch dafür, dass alles am richtigen
Ort ist. Diese Analogie hilft dir, die oft abstrakten Konzepte in alltägliche Szenarien zu übersetzen
und so schneller ein mentales Modell von Kubernetes aufzubauen.

| Kubernetes-Konzept | Analogie                 | Beschreibung                                                              |
| ------------------ | ------------------------ | ------------------------------------------------------------------------- |
| [**Cluster**](/docs/kubernetes-basis/glossar#cluster)        | Gesamtes Logistikzentrum | Alle Maschinen und Ressourcen, die zusammenarbeiten                       |
| [**Control Plane**](/docs/kubernetes-basis/glossar#control-plane)  | Büro / Zentrale          | Trifft alle Entscheidungen: was läuft wo, was passiert bei Problemen      |
| [**Worker Node**](/docs/kubernetes-basis/glossar#worker-node)    | Arbeitsplatz / LKW       | Die Maschine, auf der Container tatsächlich laufen                        |
| [**Pod**](/docs/kubernetes-basis/glossar#pod)            | Paket / Liefereinheit    | Kleinste deploybare Einheit, enthält einen oder mehrere Container         |
| [**Deployment**](/docs/kubernetes-basis/glossar#deployment)     | Arbeitsanweisung         | Beschreibt den gewünschten Zustand ("2 Chromium-Instanzen sollen laufen") |
| [**Service**](/docs/kubernetes-basis/glossar#service)        | Telefonnummer            | Stabile Adresse, über die eine Anwendung erreichbar bleibt                |
| [**Namespace**](/docs/kubernetes-basis/glossar#namespace)      | Abteilung                | Logische Trennung innerhalb eines Clusters                                |
...
| Komponente                  | Funktion                                                     |
| --------------------------- | ------------------------------------------------------------ |
| [**kube-apiserver**](/docs/kubernetes-basis/glossar#kube-apiserver)          | Zentraler Einstiegspunkt. Jeder kubectl-Befehl geht hierhin. |
| [**etcd**](/docs/kubernetes-basis/glossar#etcd)                    | Key-Value-Datenbank. Speichert den gesamten Cluster-Zustand. |
| [**kube-scheduler**](/docs/kubernetes-basis/glossar#kube-scheduler)          | Entscheidet, auf welchem Node ein neuer Pod laufen soll.     |
| [**kube-controller-manager**](/docs/kubernetes-basis/glossar#kube-controller-manager) | Gleicht Ist-Zustand ständig mit Soll-Zustand ab.             |
...
| Komponente            | Funktion                                                          |
| --------------------- | ----------------------------------------------------------------- |
| [**kubelet**](/docs/kubernetes-basis/glossar#kubelet)           | Agent auf jedem Node. Sorgt dafür, dass Container in Pods laufen. |
| [**kube-proxy**](/docs/kubernetes-basis/glossar#kube-proxy)        | Netzwerk-Proxy. Verwaltet Netzwerkregeln für Services.            |
| [**Container Runtime**](/docs/kubernetes-basis/glossar#container-runtime) | Führt Container aus (z.B. containerd, CRI-O).                     |
...
| ------------- | --------------------------------- | ------------------------------- |
| [**ClusterIP**](/docs/kubernetes-basis/glossar#clusterip) | Nur innerhalb des Clusters        | Standard, interne Kommunikation |
| [**NodePort**](/docs/kubernetes-basis/glossar#nodeport)  | Von außen über `<Node-IP>:<Port>` | Entwicklung, Tests, Schulungen  |
...
6. **Ist die Datei `test.txt` noch da?** -- NEIN! Ein [`emptyDir`](/docs/kubernetes-basis/glossar#emptydir)-Volume wird gelöscht, wenn der Pod
   stirbt. In Tag 2 lösen wir das mit [PersistentVolumeClaims (PVCs)](/docs/kubernetes-basis/glossar#persistentvolumeclaim)!
...
| [ReplicaSets](/docs/kubernetes-basis/glossar#replicaset) | 2      | je 1 pro Deployment                         |
...
Morgen machen wir eure Deployments **produktionsreif**: [Resource Limits](/docs/kubernetes-basis/glossar#resource-limits), Health Checks und
[Rolling Updates](/docs/kubernetes-basis/glossar#rollingupdate). Dann lagern wir Konfiguration in
[**ConfigMaps**](/docs/kubernetes-basis/glossar#configmap) und [**Secrets**](/docs/kubernetes-basis/glossar#secret) aus, schauen uns
**Security-Grundlagen** an, deployen Home Assistant und Nextcloud mit
[**persistentem Speicher** (PVCs)](/docs/kubernetes-basis/glossar#persistentvolumeclaim) und richten
[**Ingress-Routing**](/docs/kubernetes-basis/glossar#ingress) für alle vier Anwendungen ein.
