...
async function createPullRequest(sourceBranch, targetBranch, strategy) {
  ...
  try {
    // Verwende echo mit pipe um Shell-Escaping-Probleme zu vermeiden
    const prUrl = exec(
      `echo ${JSON.stringify(prBody)} | gh pr create --base "${targetBranch}" --head "${sourceBranch}" --title "${prTitle}" --body-file -`,
      { silent: true }
    );
