...
2. Sollte aktuell der dev oder main Branch aktiv sein, wechsle zu einem neuen Feature-Branch
...
3. Führe den linter (npm run lint) aus und behebe alle gefundenen Probleme. Es
4. Führe prettier (npm run format) aus, um den Code zu formatieren.
5. Führe vale (npm run prose) aus, um die Rechtschreibung und Grammatik zu prüfen. Behebe alle
   gefundenen Probleme.
6. Baue das Projekt lokal und stelle sicher, dass keine Fehler auftreten.
7. Wenn du Änderungen aufgrund von Lintermeldungen, Prettier oder vale vorgenommen hast, fahre bitte
   nochmal den Dev-server hoch und lass mich prüfen ob alles funktioniert wie es soll.
8. Füge alle Änderungen zu git hinzu.
9. Erstelle einen Commit mit einer sinnvollen Commit-Nachricht. Sollte git noch nicht konfiguriert
...
13. Merge den PR in den Development-Branch (dev). Sollte das nicht möglich sein, berichte mir
    darüber.
14. Ist der Merge erfogreich, switche lokal zum Development-Branch (dev) und pull die neuesten
    Änderungen.
15. Ist der Merge erfolgreich, wird das Deployment in der dev-Umgebung automatisch durch den Job
    https://github.com/Frickeldave/HomeNet/actions/workflows/host-waltraud.yaml ausgelöst. Bitte
    überprüfe nun auch den Status dieses Jobs und berichte mir, ob das Deployment erfolgreich war.
    Dass kann schon mal ein paar Minuten dauern.
