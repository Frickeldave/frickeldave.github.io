...
Portfolio) mit statischer Site-Generierung für GitHub Pages. Das Projekt kombiniert
...
- **Typed Exports**: Verwende `@/types` für typsichere Content-Queries (z.B. `BlogEntry`,
  `DocsEntry`)
...
### Automation & Deployments (WICHTIG für KI-Assistenten)

**REGEL**: Verwende NIEMALS manuelle `git commit` oder `git push` Kommandos! Nutze stattdessen IMMER
die Deployment-Skripte, da diese Issue-Creation, AI-Commit-Messages und Quality Gates bündeln!

Beide Befehle werden über ein einziges Script gesteuert: `scripts/workflows/ci/deploy.mjs`

```bash
npm run deploy:dev   # Committet, pusht auf dev (unterstützt Feature-Branches + Merge)
npm run deploy:dev -- --auto-cleanup # Non-interactive: Feature-Branch nach Merge löschen
...
**Regel**: Keine unused Variables/Parameters ohne `_` Prefix, keine `any`-Types, explizite
Return-Types
...
**Problem**: GitHub Pages unterstützt keine dynamischen Routen **Lösung**: Statische `/redirect/`
Seite + Client-side Routing
